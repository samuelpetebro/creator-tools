...
const names=new Set(['process_start','process_complete','process_error','process_cancel','download_click','cta_account','cta_plans','cta_pro_early_access','signup_success','login_success','oauth_google_start','preset_save','preset_load','preset_delete','pro_batch_use','pro_custom_pack_use','pro_creator_profile_save','pro_creator_profile_activate','pro_creator_profile_delete','pro_creator_profile_apply','pro_brand_kit_save','pro_brand_kit_activate','pro_brand_kit_delete','pro_brand_kit_apply','pro_recipe_save','pro_recipe_delete','pro_recipe_run','pro_run_again','pro_backup_export','pro_backup_import']);
...
window.DroopAnalytics=Object.freeze({start(){if(running)return;running=true;send('process_start');},finish(outcome){if(!running||!['complete','error','cancel'].includes(outcome))return;running=false;send('process_'+outcome);},track(name){if(names.has(name))send(name);}});
...
for(const [key,value] of Object.entries({'data-website-id':config.websiteId,'data-auto-pageview':'false','data-exclude-search':'true','data-exclude-hash':'true','data-do-not-track':'true','data-before-send':'droopAnalyticsBeforeSend'}))script.setAttribute(key,value);
...
document.addEventListener('click',event=>{if(event.isTrusted===false)return;const target=event.target.closest?.('[data-droop-event]');const name=target?.getAttribute('data-droop-event');if(name&&names.has(name))send(name);},true);
