...
async function loadFFmpeg(){
  ...
  const wrappers=[
    'https://cdn.jsdelivr.net/npm/@ffmpeg/ffmpeg@0.11.1/dist/ffmpeg.min.js',
    'https://unpkg.com/@ffmpeg/ffmpeg@0.11.1/dist/ffmpeg.min.js'
  ];
  let loaded=false,lastErr=null;
  for(const src of wrappers){try{await loadScript(src);if(window.FFmpeg&&window.FFmpeg.createFFmpeg){loaded=true;break;}}catch(e){lastErr=e;}}
  if(!loaded)throw lastErr||new Error('FFmpeg wrapper did not initialize in this browser.');
  ...
  for(const corePath of corePaths){
    const instance=createFFmpeg({log:false,corePath,progress:({ratio})=>{if(Number.isFinite(ratio)){const pct=Math.max(0,Math.min(100,Math.round(ratio*100)));progressBox.textContent=`Compressing… ${pct}% · keep this tab open`;}}});
...
encodeButton.addEventListener('click',async()=>{if(!file||videoKbps<80)return;encodeButton.disabled=true;downloadButton.hidden=true;progressBox.hidden=false;try{const engine=await loadFFmpeg();progressBox.textContent='Preparing video in browser memory…';const inputName=`input.${ext(file.name)}`,outputName='creator-tools-output.mp4';try{engine.FS('unlink',inputName);}catch(_){}try{engine.FS('unlink',outputName);}catch(_){}engine.FS('writeFile',inputName,await engine.__fetchFile(file));progressBox.textContent='Compressing… 0% · keep this tab open';await engine.run('-i',inputName,'-map','0:v:0','-map','0:a?','-c:v','libx264','-b:v',`${videoKbps}k`,'-maxrate',`${Math.max(videoKbps,100)}k`,'-bufsize',`${Math.max(videoKbps*2,200)}k`,'-preset','veryfast','-pix_fmt','yuv420p','-c:a','aac','-b:a',`${audioKbps}k`,'-movflags','+faststart','-y',outputName);const data=engine.FS('readFile',outputName);const blob=new Blob([data.buffer],{type:'video/mp4'});if(outputURL)URL.revokeObjectURL(outputURL);outputURL=URL.createObjectURL(blob);downloadButton.dataset.filename=`${safeBase(file.name)}-under-${String(size.value).replace('.','-')}${unit.value.toLowerCase()}.mp4`;downloadButton.hidden=false;progressBox.textContent=`Done ✓ ${bytes(blob.size)} output. ${blob.size<=targetBytes?'Target reached.':'Slightly above target; encoding size can vary a little.'}`;try{engine.FS('unlink',inputName);}catch(_){}try{engine.FS('unlink',outputName);}catch(_){}}catch(err){console.error('[CreatorTools video compression]',err);progressBox.textContent=`Compression failed: ${cleanError(err)}`;}finally{encodeButton.disabled=false;encodeButton.textContent='Compress again';}});
