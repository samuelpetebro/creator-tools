...
function cleanError(err){const raw=err&&err.message?err.message:String(err||'Unknown error');return raw.replace(/^Error:\s*/,'').slice(0,700);}
...
async function loadFFmpeg(){
  ...
  progressBox.textContent='Loading modern single-thread video engine for the first run…';
  ...
  throw new Error(`Modern single-thread video engine failed to load. ${cleanError(lastErr)}`);
}
