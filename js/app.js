(() => {
  ...
  function loadMakeItFitImage(file){
    ...
    reader.onerror=()=>{
      currentImage=null;
      alert('Could not read this file. Please try another image.');
    };
  ...
  function buildExport(){
    if(!currentImage){
      alert(currentFile?'The image is still loading. Try again in a moment.':'Upload an image first.');
      return false;
    }
  ...
  processBtn.addEventListener('click',()=>{
    if(buildExport())requestAnimationFrame(()=>result.scrollIntoView({behavior:'smooth',block:'nearest'}));
  });
