...
function boot(options={}){
 ...
 const document={documentElement:{lang:'es'},referrer:'https://search.example/private/path?secret=name',head:{appendChild:s=>scripts.push(s)},createElement:()=>({attrs:{},setAttribute(k,v){this.attrs[k]=v}}),querySelector:()=>null,addEventListener:(k,v)=>(listeners[k]??=[]).push(v)};
...
const b=boot();assert.equal(b.scripts[0].attrs['data-auto-pageview'],'false');assert.equal(b.scripts[0].attrs['data-auto-track'],undefined);b.window.DroopAnalytics.start();b.window.DroopAnalytics.start();b.window.DroopAnalytics.finish('complete');b.window.DroopAnalytics.finish('complete');b.scripts[0].onload();assert.deepEqual(b.requests.map(p=>p.name),[undefined,'process_start','process_complete']);
const trustedDownload={isTrusted:true,target:{closest:selector=>selector.includes('.download-button')?{matches:()=>false,getAttribute:()=>''}:null}};
for(const listener of b.listeners.click||[])listener(trustedDownload);
assert.equal(b.requests.at(-1).name,'download_click');
const beforeSynthetic=b.requests.length;
