...
function boot(options={}){
 ...
 const document={documentElement:{lang:'es'},referrer:'https://search.example/private/path?secret=name',head:{appendChild:s=>scripts.push(s)},createElement:()=>({attrs:{},setAttribute(k,v){this.attrs[k]=v}}),querySelector:()=>null,addEventListener:(k,v)=>listeners[k]=v};
...
console.log('PASS disabled defaults, privacy signals, manual pageviews, payload allowlist, queued event order, deduplication, provider failure isolation');
