...
const audioHtml=fs.readFileSync('audio-converter.html','utf8');
...
const audioCore=fs.readFileSync('js/tools/audio-converter-standalone.js','utf8');
const audioBatch=fs.readFileSync('js/tools/audio-converter-batch.js','utf8');
...
assert.doesNotThrow(()=>new Function(audioCore),'Audio Converter core must remain valid JavaScript');
assert.doesNotThrow(()=>new Function(audioBatch),'Audio Converter batch client must remain valid JavaScript');
assert.doesNotThrow(()=>new Function(releasePro),'custom Release Pack client must remain valid JavaScript');
...
assert(audioHtml.includes('data-pro-feature="audio-converter-batch"'),'Audio Converter must expose a Pro batch panel');
assert(audioHtml.includes('js/pro-access.js?v=1'),'Audio Converter must load the shared Pro entitlement helper');
assert(audioHtml.includes('js/batch-zip.js?v=1'),'Audio Converter must load the local ZIP helper');
assert(audioHtml.includes('js/tools/audio-converter-batch.js?v=1'),'Audio Converter must load its batch behavior');
assert(audioCore.includes('DroopAudioConvertCore'),'Audio Converter must expose its existing FFmpeg core for sequential batch reuse');
assert(audioBatch.includes('const MAX_FILES=10'),'Audio batch must cap one run at 10 files');
assert(audioBatch.includes('MAX_BYTES=100*1024*1024'),'Audio batch must cap total input size');
assert(audioBatch.includes("access?.isPro"),'Audio batch processing must require Pro at execution time');
assert(audioBatch.includes("track?.('pro_batch_use')"),'Audio batch must record privacy-safe Pro activation');
...
assert(releaseHtml.includes('data-pro-feature="release-pack-custom"'),'Release Pack must expose the Pro custom pack builder');
assert(releaseHtml.includes('js/tools/release-pack-pro.js?v=1'),'Release Pack must load the Pro custom pack behavior');
assert(releasePro.includes("access?.isPro"),'custom Release Pack must require Pro at execution time');
assert(releasePro.includes("track?.('pro_custom_pack_use')"),'custom Release Pack must record privacy-safe Pro activation');
...
assert(analytics.includes("'pro_batch_use'")&&analytics.includes("'pro_custom_pack_use'")&&analytics.includes("'pro_backup_export'")&&analytics.includes("'pro_backup_import'"),'Pro feature events must remain analytics-allowlisted');
