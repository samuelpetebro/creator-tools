...
as $$ begin insert into public.profiles (id,email) values (new.id,new.email)
...
create table if not exists public.presets (
  ...
  tool_slug text not null, name text not null,
...
create trigger presets_set_updated_at before update on public.presets for each row execute procedure public.set_updated_at();
create or replace function public.enforce_preset_limit()
returns trigger language plpgsql security definer set search_path = public
as $$ declare account_plan text; preset_count bigint; preset_limit integer;
begin
...
if preset_count >= preset_limit then raise exception 'Preset limit reached for % plan (% presets).',coalesce(account_plan,'free'),preset_limit; end if;
return new; end; $$;
drop trigger if exists presets_enforce_limit on public.presets;
create trigger presets_enforce_limit before insert on public.presets for each row execute procedure public.enforce_preset_limit();
